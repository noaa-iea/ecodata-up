...
server <- function(input, output, session) {
  ...
  git_branch_freshen <- function(){
    ...
    if (dir.exists(values$dir_ecodata_branch)){
      ...
      git_cmds <- glue(
        ...
        chmod -R 775 ../{values$dir_ecodata_branch}
  ...
  load_data <- function(dataset){
    ...
    load_log <- glue("{values$dir_ecodata_branch}/data-raw/get_{dataset$dataset_code}_log.txt")
  ...
  output$figs <- renderUI({
    ...
    plot_chunk_to_tab_panel <- function(plot_chunk){
      ...
      dir_create(dirname(plot_pfx), mode = "u=rw,go=rw")
